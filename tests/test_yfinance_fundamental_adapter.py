...
def _recent_dividend_dates() -> pd.DatetimeIndex:
    now = pd.Timestamp.now(tz="America/New_York").normalize()
    return pd.DatetimeIndex(
        [now - pd.Timedelta(days=days) for days in (270, 180, 90, 30)]
    )
...
class TestYfinanceFundamentalAdapter(unittest.TestCase):
    def test_populates_growth_earnings_dividend_boards_for_us_stock(self) -> None:
        ...
        dividend_dates = _recent_dividend_dates()
        dividends = pd.Series(
            [0.26, 0.26, 0.26, 0.27],
