...
def _price_source(as_of: str = "2026-02-03T16:00:00+08:00") -> dict:
    ...
    return {
        ...
        "benchmark_reference_snapshots": [
            {"captured_at": "2026-01-05T09:58:00+08:00", "price": 100.0}
        ],
        "signal_snapshots": {
            "600100": {"captured_at": "2026-01-05T09:58:00+08:00", "limit_up_price": 101.0}
        },
        "stocks": {
            "000300": {"prices": deepcopy(normal), "corporate_actions": [], "data_conflicts": []},
...
def _empty_signal_archive(root: Path) -> Path:
    batch = root / "2026" / "01" / "05" / "batch-empty"
    batch.mkdir(parents=True)
    payload = {"signals": []}
    json_path = batch / "signals.json"
    json_path.write_text(json.dumps(payload), encoding="utf-8")
    parquet_path = batch / "signals.parquet"
    _fake_parquet(parquet_path, [])
    manifest = {
        "signal_count": 0,
        "signal_ids": [],
        "files": {
            "signals.json": hashlib.sha256(json_path.read_bytes()).hexdigest(),
            "signals.parquet": hashlib.sha256(parquet_path.read_bytes()).hexdigest(),
        },
    }
...
class TestResearchOutcomes(unittest.TestCase):
    ...
    def test_manual_normal_case_return_upside_adverse_and_true_drawdown(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            one = by_horizon[1]
            self.assertEqual(one["gross_return"], 0.04)
            self.assertEqual(one["mfe"], 0.05)
            self.assertEqual(one["mae"], -0.02)
            self.assertEqual(one["max_drawdown"], 0.0)
            three = by_horizon[3]
            self.assertEqual(three["gross_return"], -0.04)
            self.assertEqual(three["mfe"], 0.08)
            self.assertEqual(three["mae"], -0.05)
            self.assertEqual(three["horizon_high_date"], "2026-01-08")
            self.assertEqual(three["horizon_low_date"], "2026-01-09")
            self.assertAlmostEqual(three["max_drawdown"], -0.1028037383, places=9)
            five = by_horizon[5]
            self.assertEqual(five["gross_return"], 0.10)
            self.assertEqual(five["mfe"], 0.12)
            self.assertEqual(five["mae"], -0.06)
            self.assertAlmostEqual(five["max_drawdown"], -0.1028037383, places=9)

    def test_cost_scenario_benchmark_and_excess_return(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            records = self._records(
                self._run(root, self._signals(root), _price_source(), round_trip_cost_bps=50)
            )
            one = next(row for row in records if row["horizon_days"] == 1)
            self.assertEqual(one["gross_return"], 0.04)
            self.assertEqual(one["net_return"], 0.0348)
            self.assertEqual(one["benchmark_status"], "complete")
            self.assertEqual(one["benchmark_return"], 0.04)
            self.assertEqual(one["excess_return"], 0.0)
            self.assertLessEqual(
                datetime.fromisoformat(one["benchmark_reference_at"]),
                datetime(2026, 1, 5, 9, 59, tzinfo=SHANGHAI_TZ),
            )

    def test_adjusted_price_basis_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            prices = _price_source()
            prices["price_basis"] = "forward_adjusted"
            with self.assertRaisesRegex(OutcomeValidationError, "raw_unadjusted"):
                self._run(root, self._signals(root), prices)

    def test_late_or_missing_benchmark_reference_does_not_block_stock(self) -> None:
        for snapshots in (
            [],
            [{"captured_at": "2026-01-05T10:00:00+08:00", "price": 100.0}],
        ):
            with self.subTest(snapshots=snapshots), tempfile.TemporaryDirectory() as temp:
                root = Path(temp)
                prices = _price_source()
                prices["benchmark_reference_snapshots"] = snapshots
                one = next(
                    row
                    for row in self._records(self._run(root, self._signals(root), prices))
                    if row["horizon_days"] == 1
                )
                self.assertEqual(one["gross_return"], 0.04)
                self.assertEqual(one["benchmark_status"], "unavailable")
                self.assertIsNone(one["benchmark_return"])
                self.assertIsNone(one["excess_return"])

    def test_empty_signal_batch_is_valid_and_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            signals = _empty_signal_archive(root / "signals")
            first = self._run(root, signals, _price_source())
            second = self._run(root, signals, _price_source())
            self.assertEqual(first.status, "created")
            self.assertEqual(second.status, "exists")
            self.assertEqual(first.outcome_count, 0)

    def test_duplicate_signal_id_across_archives_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            signals = self._signals(root)
            original = next(signals.rglob("batch-*"))
            shutil.copytree(original, original.parent / "batch-duplicate")
            with self.assertRaisesRegex(OutcomeValidationError, "duplicate signal_id"):
    ...
    def test_unmatured_horizons_remain_pending_with_fixed_time(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            for horizon in (5, 10, 20):
                ...
                self.assertIsNone(by_horizon[horizon]["gross_return"])
                self.assertIsNone(by_horizon[horizon]["max_drawdown"])
    ...
    def test_target_session_before_close_is_pending(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIsNone(three["gross_return"])
    ...
    def test_synchronized_payload_tampering_is_detected_and_not_overwritten(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            payload["outcomes"][0]["gross_return"] = 999.0
    ...
    def test_changed_calculation_version_creates_distinct_ids_and_batch(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            prices = _price_source()
            first = self._run(root, signals, prices)
            second = self._run(root, signals, prices, calculation_version="V2.2-OUTCOME-99")
            self.assertNotEqual(first.archive_dir, second.archive_dir)
    ...
    def test_real_parquet_writer_when_research_dependency_is_available(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("max_drawdown", table.column_names)
