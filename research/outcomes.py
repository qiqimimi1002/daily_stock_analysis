...
OUTCOME_SCHEMA_VERSION = "V2.2.1"
DEFAULT_CALCULATION_VERSION = "V2.2-OUTCOME-1"
...
def _window_metrics(reference_price: float, bars: Sequence[Mapping[str, Any]]) -> Tuple[float, float, float]:
    max_upside = max([0.0] + [float(bar["high"]) / reference_price - 1.0 for bar in bars])
    max_adverse = min([0.0] + [float(bar["low"]) / reference_price - 1.0 for bar in bars])
    ...
    return _round_pct(max_upside), _round_pct(max_adverse), _round_pct(max_drawdown)
...
def _calculate_signal(
    ...
    *,
    ...
    signal_bar = stock["bars"].get(signal_date)
    signal_near_limit_up: Optional[bool] = None
    if signal_bar and signal_bar.get("limit_up_price") is not None:
        signal_near_limit_up = reference_price >= float(signal_bar["limit_up_price"]) * 0.99
    ...
    for horizon in HORIZONS:
        ...
        if mature and target_is_available and target_bar is not None and not target_bar["is_suspended"] and not window_conflicts:
            ...
            future_return = _round_pct(horizon_close / reference_price - 1.0)
        ...
        if mature and valid_bars and not window_conflicts:
            max_upside, max_adverse, max_drawdown = _window_metrics(reference_price, valid_bars)
