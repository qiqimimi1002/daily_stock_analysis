...
> **GitHub Actions：** 出于 Public 模型端与 Private 采集端的边界要求，仓库自带 workflow 不会把 `TUSHARE_TOKEN` 或 `TUSHARE_HTTP_URL` 注入任务环境；上表中的 Tushare 配置仅适用于本地、Docker 或独立的 Private 采集端。`00-daily-analysis.yml` 仍会把 `TICKFLOW_API_KEY` / `TICKFLOW_*` 和上表中的 `LONGBRIDGE_*` 映射到任务环境。TickFlow 的 API Key 建议放在 **Secrets**，优先级、复权和批量开关可放在 **Variables** 或 **Secrets**。Longbridge OAuth 方式需要一个 client_id（优先 `LONGBRIDGE_OAUTH_CLIENT_ID`；留空且无 Legacy Access Token 时使用 `LONGBRIDGE_APP_KEY` 兼容），并把本机 `~/.longbridge/openapi/tokens/<client_id>` 文件 base64 后保存为 Secret `LONGBRIDGE_OAUTH_TOKEN_CACHE_B64`；Legacy 方式仍可配置 `LONGBRIDGE_APP_KEY`、`LONGBRIDGE_APP_SECRET`、`LONGBRIDGE_ACCESS_TOKEN`。可选接入点变量（如 `LONGBRIDGE_REGION`）可放在 **Variables** 或 **Secrets**。

> **Public workflow 边界：** `00-daily-analysis.yml` 与 `01-market-screening.yml` 均不映射 `TUSHARE_TOKEN` 或 `TUSHARE_HTTP_URL`。仅在本地、Docker 或独立 Private 采集端显式注入这些变量时，Tushare fetcher 才可能读取它们；在仓库自带 Public Actions 中新增同名 Secret / Variable 不会把凭证或自定义端点传入运行进程。
