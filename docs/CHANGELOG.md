...
- [改进] 将 DoubleEnsemble prospective shadow 前移到收盘后完成 T 日全市场准备、下一真实交易日 Top5 与最少技术字段不可变归档；次日只读前夜候选并手动触发 Private `rt_k` 行情确认。
...
- [新功能] 新增离线公开选股模型统一赛马框架 Phase 1，冻结模型/信号身份、严格时点契约与 V2.1 Universe 适配接口，不包含具体模型或未来收益计算。
